class ratInaMaze {
        ...
        if(row< board.length && col<board[row].length && board[row][col]==1) return true;
        return false;
    }
    ...
    static boolean ratInAMaze(int board[][], int row, int col, int [][]path ){
        ...
        if(isNotBlocked(board, row, col)){
                path[row][col] = 1; 
                if(ratInAMaze(board, row+1, col, path)) return true;
                if(ratInAMaze(board, row, col+1, path)) return true;
                path[row][col] = 0;
             }
             return false;
